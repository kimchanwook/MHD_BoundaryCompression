...
\section{Peak-time, final-time, and time-averaged scores}
...
\begin{enumerate}
    ...
    \item \textbf{time-averaged objective:} the average score over a selected time window.
\end{enumerate}

Each reveals something different. Peak score favors short-lived optimum compression events. Final score favors more persistent end states. Time average favors sustained good behavior.
...
\begin{keybox}
\textbf{Practical rule.} Compression metrics answer \emph{how much}. Uniformity and symmetry metrics answer \emph{how well}. The objective function is meaningful only if both categories are represented explicitly.
\end{keybox}
