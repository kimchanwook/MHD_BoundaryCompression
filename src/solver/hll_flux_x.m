function F = hll_flux_x(U, physics, numerics)
[UL, UR] = reconstruct_plm(U, 'x', physics, numerics);
FL = ideal_mhd_flux_x(UL, physics, numerics);
FR = ideal_mhd_flux_x(UR, physics, numerics);

pL = conservative_to_primitive(UL, physics, numerics);
pR = conservative_to_primitive(UR, physics, numerics);

cfL = compute_fast_speed(pL.rho, pL.p, pL.Bx, pL.By, pL.Bz, 'x', physics);
cfR = compute_fast_speed(pR.rho, pR.p, pR.Bx, pR.By, pR.Bz, 'x', physics);
ch = compute_glm_ch(U, grid_dummy_from_size(U), physics, numerics);

SL = min(min(pL.vx - cfL, pR.vx - cfR), -ch);
SR = max(max(pL.vx + cfL, pR.vx + cfR), +ch);
